package com.gg.tetris.block.app.game.command.figure.s_command
    ...
    override fun getFigureState(
        ...
        val (containerState, originalState) = Pair(
            FigureItem.ContainerParams(
                width = ((config.containerBlockSize * 2f) + config.containerBlockOffset).toInt(),
                height = ((config.containerBlockSize * 3f) + config.containerBlockOffset * 2).toInt(),
                blocks = containerBlocks,
            ),

            FigureItem.OriginalParams(
                width = ((config.originalBlockSize * 2f) + config.originalBlockOffset).toInt(),
                height = ((config.originalBlockSize * 3f) + config.originalBlockOffset * 2).toInt(),
